...
//                $item["thumd"] = Storage::disk(config('admin.upload.disk'))->url($item["pictures"][0])."?x-oss-process=image/resize,w_500";
    ...
    public function details(Request $request)
    {
        if($request->isMethod("post")){
            ...
            $res = Housings::find($data["house_id"],["id","rentsale","title","price","area","agent_id","floor_id","city_id","district_id","circle_id","purpose","years","direction","room","hall","toilet","renovation","floor","t_floor", DB::raw('price/area AS unit_price'),"pictures","desc"]);
